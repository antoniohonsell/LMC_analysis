...
# my New function for ResNet

def compute_nearest_neighbors(feats, topk=1, chunk_size=None):
    """
    Compute nearest neighbors of feats (N x D) using inner product similarity.
    Memory-safe option via chunking.

    Args:
        feats: torch.Tensor of shape [N, D]
        topk: int
        chunk_size: int or None. If set, computes in chunks of rows.
    Returns:
        knn: LongTensor of shape [N, topk]
    """
    ...
    if chunk_size is None or chunk_size >= n:
        sims = feats @ feats.T
        sims.fill_diagonal_(-1e8)
        return torch.topk(sims, k=topk, dim=1).indices

    knn_chunks = []
    for start in range(0, n, chunk_size):
        end = min(start + chunk_size, n)
        sims = feats[start:end] @ feats.T  # [chunk, N]

        # exclude self matches for rows in this chunk
        row = torch.arange(end - start, device=device)
        sims[row, start + row] = -1e8

        knn_chunks.append(torch.topk(sims, k=topk, dim=1).indices)
...
class AlignmentMetrics:
    ...
    def mutual_knn(feats_A, feats_B, topk):
        """
        Mutual-KNN overlap per sample, averaged across samples.
        Memory-safe: avoids N x N masks.
        ...
        Returns:
            float
        """
        knn_A = compute_nearest_neighbors(feats_A, topk=topk)
        knn_B = compute_nearest_neighbors(feats_B, topk=topk)

        # overlap for each row i: |KNN_A(i) ∩ KNN_B(i)| / topk
        # shape: (N, topk, topk) -> any over last dim -> (N, topk) -> mean -> (N,)
        overlap = (knn_A.unsqueeze(2) == knn_B.unsqueeze(1)).any(dim=2).float().mean(dim=1)
    ...
    def cka(feats_A, feats_B, kernel_metric='ip', rbf_sigma=1.0, unbiased=False):
        """
        CKA between features.

        Fast path for linear (inner-product) CKA when unbiased=False:
        CKA = ||X^T Y||_F^2 / (||X^T X||_F * ||Y^T Y||_F)
        """
        if kernel_metric == 'ip' and not unbiased:
            ...
            num = (XTY ** 2).sum()
            den = torch.sqrt((XTX ** 2).sum() * (YTY ** 2).sum()) + 1e-6
            return (num / den).item()
